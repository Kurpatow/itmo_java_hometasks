...
public class King extends Unit {
    ...
    public void createArmy() {
        if (gold < AppSetting.ARMY_PRICE) {
            System.out.println("Стоимость армии: " + AppSetting.ARMY_PRICE + "У короля есть: " + gold);
        }
    ...
    public void armyRenewal() {
        for (int i = 0; i < army.length; i++) {
            if (gold < AppSetting.UNIT_PRICE) {
                System.out.println("Стоимость юнита: " + AppSetting.UNIT_PRICE + "У короля есть: " + gold);
            }
            if (!army[i].isAlive()) {
